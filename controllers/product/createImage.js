...
router.post('/generateImage', validation, upload.fields([
  ...
  try {
    if (!req.files?.person || !req.files?.garment) {
      return res.sendJson({
        ...
        err: "'person' and 'garment' images are required.",
      });
    }
    ...
    const image = await ProductService.generateImage({ personPath, garmentPath, prompt: userPrompt });
