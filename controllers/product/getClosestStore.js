...
router.post('/getClosestStore', validation, async (req, res) => {
  try {
    const { userPincode } = req.body;
    if (!userPincode) {
        return res.status(400).json({ error: 'Missing required field: userPincode' });
    }
    const closestStore = await ProductService.getClosestStore(userPincode);
    res.sendJson({
